...
stream.on('tweet', function (tweet) { // listen for tweet that matches track phrase
...
async function getTweetContent(status, replyTweet, header, twToTwtch){ // get content of tweet (replied to) to twetch
    T.get('statuses/show/:id', {id:status, tweet_mode: 'extended'}, async function(err, data, response){
...
async function tncPowLink(url){
    ...
    try{res = await TonicPow.createLink({"target_url":url})}
    catch(e){console.log(`Error while fetching TonicPow short link for url ${url}, failed to reply on Twitter. `, e);return}
    return res.short_link_url;
}
async function resTweet(requestor, reply, url){
    let twetchURL;
    if (process.env.useTncPow === '1'){twetchURL = await tncPowLink(url)}
    else {twetchURL = url}
...
Link to post 👇
${twetchURL}`;
    T.post('statuses/update', {status: twtContent, in_reply_to_status_id: reply}, function(err, data, response){
        if (response.statusCode === 200){
            console.log(`Tweet successfully posted at: ${process.env.twitterURL}${process.env.twetchDat}/status/${data.id_str}`);
        } else{console.log(`Error while posting reply to Tweet ${process.env.twitterURL}${requestor}/status/${reply}, failed to reply to @${requestor} on Twitter. `, err);return}
    })
}
